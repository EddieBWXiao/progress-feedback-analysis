function [fitted,BIC] = analysis_fit_DMprogress
...
keepIDs = readtable('data/keep_effort_not_failed.csv'); %just the IDs
ptpIDs = keepIDs.x;
...
for ptp = 1:nptp
    ...
    df = readtable(sprintf('data/Exp2DecisionMaking/DM%i.csv',ptpIDs(ptp))); %DM for all decision-making
    fprintf('fitting %i \n',ptpIDs(ptp))
    ...
    for j = 1:nmodels
        ...
        fitted.prolific_id{ptp} = ptpIDs(ptp); %IMPORTANT: store ptp sequence
